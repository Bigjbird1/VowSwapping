...
const fixImagesFormat = () => {
  // Get all products
  db.all('SELECT id, images FROM Product', (err, rows) => {
    if (err) {
    ...
    rows.forEach(product => {
      try {
        ...
        // Parse the current images
        let images;
        try {
          // Try to parse if it's a JSON string
          images = JSON.parse(product.images);
        } catch (e) {
          // If it's not valid JSON, use a default array
          console.log('Invalid JSON, using default array');
          images = [];
        }
        ...
        const fixedImages = JSON.stringify(images);
