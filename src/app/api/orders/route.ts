  ...
  try {
    ...
    if (error && typeof error === 'object') {
      ...
      if (typeof error.message === 'string') {
        const lowerMessage = error.message.toLowerCase(); // Add this line

        if (lowerMessage.includes('payment') && lowerMessage.includes('verification')) {
        ...
        // Add database connection/operation error detection
    ...
    // Default to standardized database message
    return NextResponse.json(
      { error: 'Database connection failed. Please try again later.' },
      { status: 500 }
    );
